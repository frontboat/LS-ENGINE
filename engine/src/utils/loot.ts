...
const ItemString: { [key: number]: string } = Object.entries(ItemId).reduce(
  (acc, [name, id]) => {
...
export const ItemUtils = {
  ...
  getItemType: (id: number): ItemType => {
    ...
    if (ItemUtils.isMagicOrCloth(id))
      return ItemUtils.isWeapon(id) ? ItemType.Magic : ItemType.Cloth;
    if (ItemUtils.isBladeOrHide(id))
      return ItemUtils.isWeapon(id) ? ItemType.Blade : ItemType.Hide;
    if (ItemUtils.isBludgeonOrMetal(id))
      return ItemUtils.isWeapon(id) ? ItemType.Bludgeon : ItemType.Metal;
  ...
  getSpecialsSeed: (itemId: number, entropy: number): number => {
    ...
    const itemName = Object.entries(ItemId).find(
    ...
    switch (slot) {
      case "Weapon":
        slotLength = ItemSlotLength.SlotItemsLengthWeapon;
        break;
      case "Chest":
        slotLength = ItemSlotLength.SlotItemsLengthChest;
        break;
      case "Head":
        slotLength = ItemSlotLength.SlotItemsLengthHead;
        break;
      case "Waist":
        slotLength = ItemSlotLength.SlotItemsLengthWaist;
        break;
      case "Foot":
        slotLength = ItemSlotLength.SlotItemsLengthFoot;
        break;
      case "Hand":
        slotLength = ItemSlotLength.SlotItemsLengthHand;
        break;
      case "Neck":
        slotLength = ItemSlotLength.SlotItemsLengthNeck;
        break;
      case "Ring":
        slotLength = ItemSlotLength.SlotItemsLengthRing;
        break;
    }
  ...
  isNameMatch: (
    ...
    if (
      itemLevel < PREFIXES_UNLOCK_GREATNESS ||
    ...
    return (
      specials.prefix === beast.specialPrefix ||
      specials.suffix === beast.specialSuffix
    );
  },
  ...
  getEquippedItemStats: (adventurer: Adventurer, bag: Item[]): Stats => {
    ...
    for (const item of bag) {
      bagStats = ItemUtils.fullItemBoost(
        item,
  ...
  getJewelryEffect: (id: number): string => {
    switch (id) {
      // Necklaces
      case 1:
        return "Pendant increases hide armor defense (3% per level)";
      case 2:
        return "Necklace increases metal armor defense (3% per level)";
      case 3:
        return "Amulet increases cloth armor defense (3% per level)";

      // Rings
      case 4:
        return "Crit chance gained from Silver Ring is doubled when equipped";
      case 6:
        return "Platinum Ring increases name match bonus damage (3% per level)";
      case 7:
        return "Titanium Ring increases critical hit bonus damage (3% per level)";
      case 8:
        return "Gold Ring increases gold earned from defeating beasts (3% extra per level)";
  ...
  getCurrentJewelryEffect: (id: number, level: number): string => {
    switch (id) {
      // Necklaces
      case 1:
        return `Hide armor defense increased by ${level * 3}%`;
      case 2:
        return `Metal armor defense increased by ${level * 3}%`;
      case 3:
        return `Cloth armor defense increased by ${level * 3}%`;

      // Rings
      case 4:
        return `Increases critical hit chance by ${level}%`;
      case 6:
        return `Name match bonus damage increased by ${level * 3}%`;
      case 7:
        return `Critical hit bonus damage increased by ${level * 3}%`;
      case 8:
        return `Gold earned from defeating beasts increased by ${level * 3}%`;
